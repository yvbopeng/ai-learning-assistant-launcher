...
export default async function init(ipcMain: IpcMain) {
  ...
  ipcHandle(ipcMain, getActiveTorrentsHandle, async (_event) =>
    getActiveTorrents(),
  );
  ipcHandle(ipcMain, installUpdateHandle, async (_event, zipPath: string) =>
    installUpdate(zipPath),
  );
...
export async function startWebtorrent(magnet: string) {
  ...
  if (dLCInfo) {
    ...
    // 如果本地索引找不到，从 magnet 链接中提取信息作为默认路径
    console.warn('本地索引找不到，使用默认下载路径', magnet);
    try {
      const url = new URL(magnet);
      const dn = url.searchParams.get('dn') || 'unknown';
      // 从文件名中提取版本号，如 AI-Learning-Assistant-Launcher-win32-x64-2.0.1-p2p.zip -> 2.0.1
      const versionMatch = dn.match(/(\d+\.\d+\.\d+)/);
      ...
      // 如果解析失败，使用默认路径
      filePath = path.join(appPath, 'external-resources', 'dlc', 'downloads');
    }
  }
    ...
    return new Promise((resolve) => {
      client.add(
        ...
        (newTorrent) => {
          ...
          console.debug('Client is downloading:', newTorrent.infoHash);
          console.debug(`[add] 开始下载 ${newTorrent.name}`);
          console.debug(`[add] 存储地址 ${newTorrent.path}`);
          saveTorrentFile(filePath, newTorrent);
          // 返回可序列化的结果对象
          resolve({
            infoHash: newTorrent.infoHash,
            name: newTorrent.name,
            path: newTorrent.path,
...
export async function installUpdate(
  ...
  try {
    console.log('开始安装更新:', zipPath);

    // 检查 zip 文件是否存在
    if (!existsSync(zipPath)) {
      return { success: false, message: `文件不存在: ${zipPath}` };
    }
    ...
    // 创建临时目录用于解压
    const tempDir = path.join(appPath, 'temp_update');
    ...
    console.log('正在解压文件...');
    const unzipCommand = `powershell -Command "Expand-Archive -Path '${zipPath}' -DestinationPath '${tempDir}' -Force"`;
    execSync(unzipCommand, { encoding: 'utf8' });
    console.log('解压完成');
    ...
    console.log('解压源目录:', sourceDir);
    console.log('目标目录:', currentDir);

    // 创建更新批处理脚本
    const batchPath = path.join(appPath, 'update.bat');
    const exeName = path.basename(currentExePath);
    // 将路径中的斜杠转为反斜杠
    const sourceDirWin = sourceDir.replace(/\//g, '\\');
    const currentDirWin = currentDir.replace(/\//g, '\\');
    const tempDirWin = tempDir.replace(/\//g, '\\');
    const newExePath = path.join(currentDir, exeName).replace(/\//g, '\\');
...
echo 正在复制新文件...
xcopy /E /Y /I "${sourceDirWin}\\*" "${currentDirWin}"

echo 清理临时文件...
rmdir /S /Q "${tempDirWin}"
